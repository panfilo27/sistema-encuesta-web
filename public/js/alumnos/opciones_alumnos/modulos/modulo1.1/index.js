/**
 * Módulo 1.1: Datos Generales - Encuesta de Química y Bioquímica
 * 
 * Este archivo maneja la lógica principal del módulo 1.1 de la encuesta especializada,
 * incluyendo la carga de datos del usuario, inicialización del formulario,
 * y la coordinación entre validación y almacenamiento de datos.
 */

// Referencia al formulario
const formulario = document.getElementById('form-modulo1-1');
const loadingModule = document.getElementById('loading-module');
const loadingOverlay = document.getElementById('loading-overlay');
const errorContainer = document.getElementById('error-container');
const btnGuardar = document.getElementById('btn-guardar-modulo1-1');
const razonNoTituladoContainer = document.getElementById('razon_no_titulado_container');

// Variables globales
let currentUser = null;
let encuestaActual = null;
let moduloCompletado = false;
let respuestasAnteriores = null;

/**
 * Inicializa el módulo cuando el DOM está listo
 */
document.addEventListener('DOMContentLoaded', async () => {
    console.log('Inicializando Módulo 1.1: Datos Generales - Química y Bioquímica');
    
    // Añadir estilos para campos bloqueados
    const style = document.createElement('style');
    style.textContent = `
        .campo-bloqueado small {
            display: block;
            margin-top: 5px;
            color: #666;
            font-style: italic;
        }
        .campo-bloqueado i {
            color: #888;
            margin-right: 5px;
        }
        select:disabled {
            background-color: #f8f8f8;
            border-color: #ddd;
            color: #555;
            cursor: not-allowed;
        }
    `;
    document.head.appendChild(style);
    
    // Verificar autenticación mediante localStorage
    const userSession = localStorage.getItem('userSession');
    
    if (userSession) {
        try {
            const userData = JSON.parse(userSession);
            
            // Verificar que la sesión sea de un alumno
            if (userData.rolUser !== 'alumno') {
                alert('No tienes permiso para acceder a esta sección.');
                window.location.href = '../../../../../auth/login.html';
                return;
            }
            
            // Verificar que el usuario sea de la carrera de Química o Bioquímica
            const userDoc = await firebase.firestore().collection('usuario').doc(userData.id).get();
            if (userDoc.exists) {
                const userData = userDoc.data();
                if (userData.carrera !== 'Química' && userData.carrera !== 'Bioquímica') {
                    alert('Esta encuesta es exclusiva para estudiantes de Química y Bioquímica.');
                    window.location.href = '../../encuestas.html';
                    return;
                }
            }
            
            currentUser = userData;
            
            // Actualizar nombre de usuario en la interfaz
            document.getElementById('nombre-usuario').textContent = userData.nombre || userData.usuario;
            
            // Evento de cierre de sesión
            document.getElementById('btn-cerrar-sesion').addEventListener('click', () => {
                localStorage.removeItem('userSession');
                window.location.href = '../../../../../auth/login.html';
            });
            
            try {
                // Cargar datos y verificar si el módulo ya fue completado
                await cargarDatos();
                
                // Generar opciones de años para fechas
                generarOpcionesDeAnios();
                
                // Si el módulo ya fue completado, habilitar modo solo visualización
                if (moduloCompletado) {
                    console.log('Este módulo ya fue completado anteriormente');
                    // Bloquear todos los campos del formulario
                    habilitarModoSoloVisualizacion();
                    // Cambiar el texto del botón
                    if (btnGuardar) {
                        btnGuardar.textContent = 'Continuar al siguiente módulo';
                    }
                }
                
                // Configurar eventos del formulario
                configurarEventos();
                
                // Mostrar formulario
                loadingModule.style.display = 'none';
                formulario.style.display = 'block';
            } catch (error) {
                console.error('Error al inicializar el módulo 1.1:', error);
                mostrarError('Error al cargar el formulario. Por favor, recarga la página.');
            }
        } catch (error) {
            console.error('Error al procesar la sesión:', error);
            localStorage.removeItem('userSession');
            window.location.href = '../../../../../auth/login.html';
        }
    } else {
        // Usuario no autenticado, redirigir al login
        alert('Debes iniciar sesión para acceder a esta sección.');
        window.location.href = '../../../../../auth/login.html';
    }
});

/**
 * Genera las opciones de años para los selectores de fechas
 */
function generarOpcionesDeAnios() {
    const selectIngreso = document.getElementById('fecha_ingreso');
    const selectEgreso = document.getElementById('fecha_egreso');
    
    const anioActual = new Date().getFullYear();
    const anioInicio = anioActual - 30; // 30 años hacia atrás
    
    for (let anio = anioActual; anio >= anioInicio; anio--) {
        // Añadir opción al selector de ingreso
        const optionIngreso = document.createElement('option');
        optionIngreso.value = anio;
        optionIngreso.textContent = anio;
        selectIngreso.appendChild(optionIngreso);
        
        // Añadir opción al selector de egreso
        const optionEgreso = document.createElement('option');
        optionEgreso.value = anio;
        optionEgreso.textContent = anio;
        selectEgreso.appendChild(optionEgreso);
    }
}

/**
 * Carga los datos necesarios para el formulario
 */
async function cargarDatos() {
    try {
        // 1. Obtener encuesta activa para Química/Bioquímica
        const fechaActual = new Date();
        
        const encuestasSnapshot = await firebase.firestore().collection('encuestas')
            .where('carrera', 'array-contains-any', ['Química', 'Bioquímica'])
            .get();
        
        const encuestasEnRango = encuestasSnapshot.docs
            .map(doc => {
                const encuesta = { id: doc.id, ...doc.data() };
                
                // Convertir Timestamps a Date si existen
                if (encuesta.fechaInicio && typeof encuesta.fechaInicio.toDate === 'function') {
                    encuesta.fechaInicio = encuesta.fechaInicio.toDate();
                }
                
                if (encuesta.fechaFin && typeof encuesta.fechaFin.toDate === 'function') {
                    encuesta.fechaFin = encuesta.fechaFin.toDate();
                }
                
                return encuesta;
            })
            .filter(encuesta => {
                // Verificar que la encuesta está en el rango de fechas válido
                const fechaInicioValida = !encuesta.fechaInicio || encuesta.fechaInicio <= fechaActual;
                const fechaFinValida = !encuesta.fechaFin || encuesta.fechaFin >= fechaActual;
                return fechaInicioValida && fechaFinValida;
            });
        
        if (encuestasEnRango.length === 0) {
            throw new Error('No hay encuestas activas para Química/Bioquímica en este momento.');
        }
        
        encuestaActual = encuestasEnRango[0];
        
        // 2. Verificar si el usuario ya completó este módulo para esta encuesta
        const historialEncuestaDoc = await firebase.firestore()
            .collection('usuario')
            .doc(currentUser.id)
            .collection('historial_encuestas')
            .doc(encuestaActual.id)
            .get();
        
        if (historialEncuestaDoc.exists) {
            const datosEncuesta = historialEncuestaDoc.data();
            
            // Verificar si existe el módulo 1.1 y está completado
            moduloCompletado = datosEncuesta.modulo1_1 && datosEncuesta.modulo1_1.completado === true;
            
            // Si hay respuestas anteriores, cargarlas
            if (moduloCompletado && datosEncuesta.modulo1_1.datos) {
                respuestasAnteriores = datosEncuesta.modulo1_1.datos;
                
                // Pre-llenar el formulario con las respuestas anteriores
                preLlenarFormulario();
            }
        } else {
            moduloCompletado = false;
        }
        
        // Cargar datos del usuario para pre-llenar algunos campos
        const userDoc = await firebase.firestore().collection('usuario').doc(currentUser.id).get();
        if (userDoc.exists) {
            const datosUsuario = userDoc.data();
            
            // Pre-llenar campos con datos del perfil del usuario
            if (datosUsuario.nombre && !formulario.nombre_completo.value) {
                formulario.nombre_completo.value = datosUsuario.nombre;
                formulario.nombre_completo.disabled = true;
                agregarIndicadorCampoBloqueado(formulario.nombre_completo);
            }
            
            if (datosUsuario.email && !formulario.email.value) {
                formulario.email.value = datosUsuario.email;
                formulario.email.disabled = true;
                agregarIndicadorCampoBloqueado(formulario.email);
            }
        }
    } catch (error) {
        console.error('Error al cargar datos:', error);
        throw error;
    }
}

/**
 * Agrega un indicador visual a un campo bloqueado
 * @param {HTMLElement} elemento - El elemento de entrada bloqueado
 */
function agregarIndicadorCampoBloqueado(elemento) {
    const parentDiv = elemento.parentElement;
    parentDiv.classList.add('campo-bloqueado');
    
    const indicador = document.createElement('small');
    indicador.innerHTML = '<i class="fas fa-lock"></i> Campo bloqueado: información del perfil';
    parentDiv.appendChild(indicador);
}

/**
 * Pre-llena el formulario con las respuestas anteriores
 */
function preLlenarFormulario() {
    if (!respuestasAnteriores) return;
    
    // Datos personales
    if (!formulario.nombre_completo.disabled) {
        formulario.nombre_completo.value = respuestasAnteriores.nombre_completo || '';
    }
    
    formulario.telefono_celular.value = respuestasAnteriores.telefono_celular || '';
    formulario.redes_sociales.value = respuestasAnteriores.redes_sociales || '';
    
    if (!formulario.email.disabled) {
        formulario.email.value = respuestasAnteriores.email || '';
    }
    
    // Información académica
    if (respuestasAnteriores.fecha_ingreso) {
        formulario.fecha_ingreso.value = respuestasAnteriores.fecha_ingreso;
    }
    
    if (respuestasAnteriores.fecha_egreso) {
        formulario.fecha_egreso.value = respuestasAnteriores.fecha_egreso;
    }
    
    if (respuestasAnteriores.titulado) {
        const radioTitulado = document.querySelector(`input[name="titulado"][value="${respuestasAnteriores.titulado}"]`);
        if (radioTitulado) radioTitulado.checked = true;
        
        // Si no está titulado, mostrar el campo de razón
        if (respuestasAnteriores.titulado === 'no') {
            razonNoTituladoContainer.style.display = 'block';
            formulario.razon_no_titulado.value = respuestasAnteriores.razon_no_titulado || '';
        }
    }
    
    if (respuestasAnteriores.trabajo_relacionado) {
        const radioTrabajoRelacionado = document.querySelector(`input[name="trabajo_relacionado"][value="${respuestasAnteriores.trabajo_relacionado}"]`);
        if (radioTrabajoRelacionado) radioTrabajoRelacionado.checked = true;
    }
}

/**
 * Configura los eventos del formulario
 */
function configurarEventos() {
    // Evento para mostrar/ocultar razón de no titulación
    const radiosTitulado = document.querySelectorAll('input[name="titulado"]');
    radiosTitulado.forEach(radio => {
        radio.addEventListener('change', () => {
            if (radio.value === 'no') {
                razonNoTituladoContainer.style.display = 'block';
                formulario.razon_no_titulado.setAttribute('required', 'required');
            } else {
                razonNoTituladoContainer.style.display = 'none';
                formulario.razon_no_titulado.removeAttribute('required');
            }
        });
    });
    
    // Evento de guardar y continuar
    btnGuardar.addEventListener('click', async () => {
        // Si el módulo ya está completado, solo navegar al siguiente módulo
        if (moduloCompletado) {
            window.location.href = 'modulo2.1.html';
            return;
        }
        
        try {
            // Mostrar overlay de carga
            loadingOverlay.style.display = 'flex';
            
            // Validar el formulario
            const resultadoValidacion = validarFormulario();
            
            if (!resultadoValidacion.valido) {
                mostrarErrores(resultadoValidacion.errores);
                loadingOverlay.style.display = 'none';
                return;
            }
            
            // Crear objeto con los datos del formulario
            const datosModulo1_1 = {
                // Datos personales
                nombre_completo: formulario.nombre_completo.value.trim(),
                telefono_celular: formulario.telefono_celular.value.trim(),
                redes_sociales: formulario.redes_sociales.value.trim(),
                email: formulario.email.value.trim(),
                
                // Información académica
                fecha_ingreso: formulario.fecha_ingreso.value,
                fecha_egreso: formulario.fecha_egreso.value,
                titulado: document.querySelector('input[name="titulado"]:checked').value,
                razon_no_titulado: formulario.titulado_no.checked ? formulario.razon_no_titulado.value : null,
                trabajo_relacionado: document.querySelector('input[name="trabajo_relacionado"]:checked').value,
                
                // Metadatos
                uid: currentUser.id,
                timestamp: new Date()
            };
            
            // Guardar en Firestore
            await guardarRespuestas(datosModulo1_1);
            
            // Redirigir al siguiente módulo
            window.location.href = 'modulo2.1.html';
        } catch (error) {
            console.error('Error al guardar el formulario:', error);
            mostrarError('Error al guardar la información. Por favor, intenta nuevamente.');
            loadingOverlay.style.display = 'none';
        }
    });
}

/**
 * Valida el formulario antes de enviar
 * @returns {Object} Resultado de la validación {valido: boolean, errores: []}
 */
function validarFormulario() {
    // Reiniciar errores
    errorContainer.innerHTML = '';
    
    const errores = [];
    
    // Validar campos requeridos
    if (!formulario.nombre_completo.value.trim()) {
        errores.push('El nombre completo es obligatorio');
    }
    
    const telefonoRegex = /^[0-9]{10}$/;
    if (!telefonoRegex.test(formulario.telefono_celular.value.trim())) {
        errores.push('El teléfono celular debe tener 10 dígitos numéricos');
    }
    
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(formulario.email.value.trim())) {
        errores.push('El correo electrónico no es válido');
    }
    
    if (!formulario.fecha_ingreso.value) {
        errores.push('La fecha de ingreso es obligatoria');
    }
    
    if (!formulario.fecha_egreso.value) {
        errores.push('La fecha de egreso es obligatoria');
    } else if (parseInt(formulario.fecha_egreso.value) < parseInt(formulario.fecha_ingreso.value)) {
        errores.push('La fecha de egreso no puede ser anterior a la fecha de ingreso');
    }
    
    // Validar radio buttons
    if (!document.querySelector('input[name="titulado"]:checked')) {
        errores.push('Debes indicar si estás titulado');
    } else if (formulario.titulado_no.checked && !formulario.razon_no_titulado.value) {
        errores.push('Debes seleccionar la razón por la que no te has titulado');
    }
    
    if (!document.querySelector('input[name="trabajo_relacionado"]:checked')) {
        errores.push('Debes indicar si tu trabajo está relacionado con tu carrera');
    }
    
    return {
        valido: errores.length === 0,
        errores: errores
    };
}

/**
 * Guarda las respuestas del módulo en Firestore
 * @param {Object} datos - Datos del módulo 1.1
 */
async function guardarRespuestas(datos) {
    try {
        const db = firebase.firestore();
        const usuarioRef = db.collection('usuario').doc(currentUser.id);
        
        // Referencia al documento de la encuesta en la subcolección historial_encuestas
        const encuestaRef = usuarioRef.collection('historial_encuestas').doc(encuestaActual.id);
        
        // Obtener el documento para ver si ya existe
        const encuestaDoc = await encuestaRef.get();
        
        if (encuestaDoc.exists) {
            // La encuesta ya existe, actualizamos solo el módulo 1.1
            await encuestaRef.update({
                'modulo1_1': {
                    datos: datos,
                    completado: true,
                    fechaCompletado: new Date()
                },
                ultimaModificacion: firebase.firestore.FieldValue.serverTimestamp()
            });
        } else {
            // Crear nuevo documento para esta encuesta
            await encuestaRef.set({
                encuestaId: encuestaActual.id,
                titulo: encuestaActual.titulo || 'Encuesta especializada para Química/Bioquímica',
                fechaCreacion: firebase.firestore.FieldValue.serverTimestamp(),
                ultimaModificacion: firebase.firestore.FieldValue.serverTimestamp(),
                tipo: 'especializada',
                carrera: currentUser.carrera || 'No especificada',
                modulo1_1: {
                    datos: datos,
                    completado: true,
                    fechaCompletado: new Date()
                }
            });
        }
        
        // Actualizar variable global
        moduloCompletado = true;
        
        // Mostrar mensaje de éxito
        alert('¡Gracias! Tus respuestas han sido guardadas correctamente.');
    } catch (error) {
        console.error('Error al guardar respuestas:', error);
        throw error;
    }
}

/**
 * Muestra mensajes de error en el contenedor
 * @param {Array} errores - Lista de mensajes de error
 */
function mostrarErrores(errores) {
    errorContainer.innerHTML = '';
    
    if (errores.length === 0) return;
    
    const ul = document.createElement('ul');
    ul.className = 'error-list';
    
    errores.forEach(error => {
        const li = document.createElement('li');
        li.textContent = error;
        ul.appendChild(li);
    });
    
    errorContainer.appendChild(ul);
    errorContainer.style.display = 'block';
    
    // Hacer scroll al contenedor de errores
    errorContainer.scrollIntoView({ behavior: 'smooth' });
}

/**
 * Muestra un mensaje de error general
 * @param {string} mensaje - Mensaje de error
 */
function mostrarError(mensaje) {
    errorContainer.innerHTML = `<p class="error-message">${mensaje}</p>`;
    errorContainer.style.display = 'block';
    errorContainer.scrollIntoView({ behavior: 'smooth' });
}

/**
 * Habilita el modo solo visualización bloqueando todos los campos del formulario
 */
function habilitarModoSoloVisualizacion() {
    console.log('Habilitando modo solo visualización');
    
    // Añadir un aviso en la parte superior del formulario
    const avisoElement = document.createElement('div');
    avisoElement.className = 'aviso-solo-visualizacion';
    avisoElement.innerHTML = `
        <div class="aviso-icono"><i class="fas fa-info-circle"></i></div>
        <div class="aviso-texto">
            <h4>Modo solo visualización</h4>
            <p>Esta encuesta ya ha sido completada. Los campos no pueden ser editados.</p>
        </div>
    `;
    
    // Insertar el aviso al inicio del formulario
    formulario.insertBefore(avisoElement, formulario.firstChild);
    
    // Seleccionar todos los elementos de entrada en el formulario
    const inputs = formulario.querySelectorAll('input, select, textarea');
    
    // Desactivar cada elemento
    inputs.forEach(input => {
        input.disabled = true;
        input.classList.add('disabled');
    });
    
    // Cambiar estilo del formulario para indicar modo visualización
    formulario.classList.add('form-readonly');
    
    // Cambiar texto del botón
    btnGuardar.textContent = 'Continuar al siguiente módulo';
    
    // Mostrar mensaje informativo en consola
    console.log('Formulario en modo solo visualización - todos los campos bloqueados');
}
